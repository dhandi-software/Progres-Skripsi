...
import { ProfileHeaderStaf } from "./components/profile-header-staf";
import { ProfileDetailsStaf } from "./components/profile-details-staf";

export default function ProfileStafDesktop() {
    const [profile, setProfile] = useState<any>(null);
    const [loading, setLoading] = useState(true);

    const fetchProfile = async () => {
        try {
            const data = await pengajuanApi.getStafProfile();
            setProfile(data);
        } catch (error) {
            console.error("Failed to fetch staf profile:", error);
        } finally {
            setLoading(false);
        }
    };
        ...
        return (
            <div className="flex flex-col items-center justify-center h-[80vh] gap-6">
                <div className="relative">
                    <Loader2 className="w-16 h-16 text-[#D25026] animate-spin" />
    ...
    return (
        <div className="min-h-screen bg-gray-50/50 pb-24">
            ...
            <div className="max-w-7xl mx-auto pt-12 px-6">
                 <ProfileHeaderStaf profile={profile} onUpdate={fetchProfile} />
            </div>

            {/* Content Section */}
            <div className="max-w-7xl mx-auto mt-16 px-6 md:px-12">
                 <ProfileDetailsStaf profile={profile} onUpdate={fetchProfile} />
            </div>

            {/* Decorative Luxury Footer */}
            <div className="mt-24 border-t border-gray-100 py-12 text-center overflow-hidden relative">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-gradient-to-b from-[#D25026]/5 to-transparent rounded-full blur-[100px] -z-10" />
