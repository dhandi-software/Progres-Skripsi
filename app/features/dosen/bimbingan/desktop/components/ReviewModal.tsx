        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white rounded-3xl shadow-2xl w-full overflow-hidden flex flex-col max-h-[96vh] border border-white/20">
                ...
                <div className="flex-1 overflow-y-auto min-h-0 flex flex-col xl:flex-row bg-gray-50/30">
                    ...
                    {/* Sidebar Actions */}
                    {!isApprovedReadOnly && (
                        <div className="w-full xl:w-[360px] p-8 flex flex-col gap-8 shrink-0 bg-white border-l border-gray-100 shadow-inner">
                            <div className="space-y-4">
                                <label className="block text-sm font-bold text-gray-800 ml-1">Keputusan Reviu</label>
                                <div className="grid grid-cols-1 gap-3">
                                    <label className={`flex items-start gap-4 p-4 rounded-2xl border-2 cursor-pointer transition-all ${reviewStatus === 'REVISION' ? 'border-orange-500 bg-orange-50/50 shadow-md' : 'border-gray-50 hover:border-gray-200 hover:bg-gray-50'}`}>
                                        <input 
                                        ...
                                        <div>
                                            ...
                                            <span className="text-[11px] text-gray-500 mt-1 block">Mahasiswa harus memperbaiki dokumen berdasarkan masukan Anda.</span>
                                        </div>
                                    </label>
                                    <label className={`flex items-start gap-4 p-4 rounded-2xl border-2 cursor-pointer transition-all ${reviewStatus === 'APPROVED' ? 'border-green-600 bg-green-50/50 shadow-md' : 'border-gray-50 hover:border-gray-200 hover:bg-gray-50'}`}>
                                        <input 
                                        ...
                                        <div>
                                            ...
                                            <span className="text-[11px] text-gray-500 mt-1 block">Target bab ini selesai dan mahasiswa dapat lanjut ke tahap berikutnya.</span>
                                        </div>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                                <div>
                                    ...
                                    <textarea 
                                        className="w-full rounded-2xl border-gray-100 border-2 p-4 text-sm focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 outline-none transition-all placeholder:text-gray-400 min-h-[120px] bg-gray-50/50"
                                ...
                                {!reviewingTask.fileMahasiswa?.toLowerCase().endsWith('.pdf') && (
                                    <div className="p-5 bg-blue-50/50 border-2 border-blue-100 rounded-2xl">
                                        ...
                                        <input 
                                            ...
                                            className="w-full text-xs text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-[10px] file:font-bold file:bg-blue-600 file:text-white hover:file:bg-blue-700 cursor-pointer bg-white border border-blue-200 rounded-2xl p-1.5 shadow-sm"
                                        />
                                    </div>
                                )}
                                
                                <div className="mt-4 p-4 border border-gray-100 rounded-2xl bg-gray-50/50">
                                    <h4 className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Versi Draf Anda Reviu</h4>
                                    <div className="flex justify-between items-center bg-white p-3 rounded-xl border border-gray-100">
                                        <div>
                                            <span className="font-bold text-gray-800 block text-sm">Versi {reviewingTask.versi}</span>
                                            <span className="text-[10px] text-gray-400 block mt-0.5">{new Date(reviewingTask.tanggal).toLocaleDateString('id-ID', {day: 'numeric', month: 'short', year: 'numeric'})}</span>
                                        </div>
